...
var Register = React.createClass({
    ...
    handleSubmit: function(event) {
        ...
        //check if user has agree on our term
        if(that.state.agreeTerm != "on"){
            console.log("term value", that.state.agreeTerm);
            return this.setState({
                feedback: 'Please agree to our term and condition'
            });
        }
            ...
            if (error) {
                ...
                .then(function(response){
                        ...
                        if(body.status === "success"){
                            ...
                            that.setState({activationEmailSent: true})
                        }
                        else
                        {
        ...
        return (
            <div>
                <form className="login-form" name="loginForm" method="post" onSubmit={this.handleSubmit}>
                    ...
                    <div className={cx({
                            'hidden': !that.state.feedback || that.state.activationEmailSent,
                    ...
                    {/* normal form -- before email sending */}
                    <div className={cx({
                            'hidden': that.state.activationEmailSent
                        })}>
                            ...
                            <ul className="social-icons">
                                <li>
                                    <a className="social-icon-color facebook"
                                       data-original-title="facebook" href="/facebook/login"></a>
                                </li>
                                <li>
                                    <a className="social-icon-color twitter"
                                       data-original-title="Twitter" href="/twitter/login"></a>
                                </li>
                                <li>
                    ...
                    {/* normal form -- after email sending */}
                    <div className={cx({
                            'hidden': !that.state.activationEmailSent,
                            'alert' : 1, 'alert-info': 1
                        })}>
                         <i className="fa fa-info-circle info"></i>
                         <span>An activation email has been sent into your provided email address.
                             If you have already activated your account, please click to the Login button below to start using our system.</span>
                    </div>
